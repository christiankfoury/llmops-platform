...
Phase 58 switched default Compose and Kubernetes packaging to Java 21/Spring Boot. Runtime acceptance is recorded in the [phase review](archive/phase-reviews/phase-58.md). Phase 61 subsequently implemented migration-aware immutable promotion and rollback workflows; they remain held for monitoring eligibility, AWS setup and explicit deployment approval. AWS is not deployed. The cutover changed no real cloud resources or secrets. See the [release runbook](immutable-release-runbook.md) and [current progress](../phases-progress.md).
...
The separate `infra/helm/ai-platform-migration` chart renders an owner Job in `ai-platform-ENV-migration`; ordinary application releases cannot create it or access that namespace. It uses its own image and `ai-platform-migration-secrets` with the three JDBC properties, a 180-second deadline and no automatic retry. Rendering it does not run it. Immutable image selection, execution and migration-aware rollback are Phase 61. Namespace, controller, SecretStore, public CA and role ownership now belong to the [ordered AWS bootstrap](aws-bootstrap.md), separate from the app chart.
...
`python scripts/validate_java_container_stack.py` requires the three prebuilt `:local` images and a Linux Docker engine. It creates a unique Compose project and short-lived synthetic PostgreSQL/Redis certificate roots under ignored `.maven-cache`, uses an empty env file, and never reads personal credentials. It checks fresh migrations, idempotent seeds, gateway accounting, all eight captured client events and replays, isolated web fixtures, private metrics, non-root/read-only execution and writable mounts. It proves PostgreSQL hostname rejection and Redis trust failure while preserving liveness, then verifies the actual API entrypoint handles SIGTERM within its allowance without an OOM or forced kill. Only this script's containers are stopped afterward; volumes are retained.
