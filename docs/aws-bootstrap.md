...
8. The bootstrap owner installs `infra/helm/ai-platform-network` as release `ai-platform-network` with its reviewed environment values before application deployment. It owns Services, Ingress and NetworkPolicies, including private metrics. Its `appReleaseName`, names and namespace must match the application release `ai-platform`; validate both releases together. The application deployer can read these objects but cannot redirect Services, change ingress or remove isolation. Network changes require bootstrap authorization and review. Raw alternatives use the separate `infra/k8s/network` base/overlays under the same ownership rule.
