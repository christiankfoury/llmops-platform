...
Phase 50 adds a separate Java CI job while Python remains the runtime. From `apps/api-java`, run `./mvnw --batch-mode --no-transfer-progress --strict-checksums clean verify` (`.\mvnw.cmd` on Windows). This verifies Java/Maven versions, release dependencies, compiler warnings, formatting and the HTTP/JSON tests. CI verifies wrapper hashes before executing Maven, then smoke-tests the packaged service. See the [Java API README](../apps/api-java/README.md) for cache setup and the tested Windows socket-directory workaround. Phase 51 adds required real PostgreSQL 16.15 tests for fresh/adopted schema parity, retained legacy rows, schema drift, JPA types, transactional uniqueness and synthetic seeds. The test harness starts a disposable native PostgreSQL instance on a random local port; failures never skip tests. CI separately runs the migration JAR before the packaged-service smoke test. The Python migration ownership regression is required against PostgreSQL in CI. See [database handover](database-migration-handover.md). Phase 56 adds required Redis integration; start the disposable fixture before Maven as described in [Java reliability](java-reliability.md).
