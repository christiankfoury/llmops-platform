...
- Phase 59 investigation (2026-09-07): replace controller-managed Ingress/ALB ownership with Terraform-owned load balancing plus restricted TargetGroupBinding reconciliation if the pinned v3.5.0 controller passes functional and authorization tests. Its Ingress reconciler cannot be switched off, so read-only discovery and admission boundaries must be tested explicitly. No scanner exception is approved or active. Cloud IAM/data-plane validation remains approval-gated.
