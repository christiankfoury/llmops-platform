...
Phases 50-58 implement the Java 21/Spring Boot service, PostgreSQL/Flyway, mock gateway, both clients' telemetry, dashboard/configuration APIs, OIDC project authorization and key lifecycle. Default Docker Compose and Helm now use Java; Python remains an explicit local reference. See the [runtime cutover guide](../../docs/java-runtime-cutover.md). AWS release workflows are updated in phases 59-61. AWS remains the cloud target. See [operator setup](../../docs/java-operator-security.md), [gateway behavior](../../docs/java-gateway.md) and [telemetry contracts](../../docs/java-telemetry.md). Distributed Redis limits and dependency readiness are implemented; see [reliability settings and required Redis fixture](../../docs/java-reliability.md). [Java metrics, structured logs and traces](../../docs/java-observability.md) are implemented. The runnable monitoring stack follows in Phase 62.

The build uses Java 21, Spring Boot 4.1.1, Maven 3.9.16, Maven Wrapper 3.3.4, Spotless 3.10.2 and Google Java Format 1.28.0. Tomcat is explicitly patched to 11.0.25 for the [upstream security fixes](https://tomcat.apache.org/security-11.html#Fixed_in_Apache_Tomcat_11.0.25); remove that override when the Boot BOM includes the patched version. The Spring Boot BOM pins other library versions; the build rejects snapshot dependencies, checks Java/Maven versions, treats compiler warnings as errors, checks formatting and runs tests. Distribution downloads have a pinned SHA-256 checksum and CI checks the committed wrapper files before executing them. Dependency vulnerability and release SBOM gates are expanded in Phase 60; checksum validation alone is not a vulnerability scan.
