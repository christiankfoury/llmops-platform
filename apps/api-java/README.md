...
The service listens on `127.0.0.1:8080`; `API_BIND_ADDRESS` and `PORT` control the bind address and port. `/health` identifies the service, `/health/live` checks lifecycle liveness, and `/health/ready` reports whether the process accepts traffic. Readiness requires fresh successful PostgreSQL/Redis checks and refuses new work during draining. Liveness remains independent of those dependencies. Only Actuator health is exposed, with details hidden; diagnostic/configuration/dump endpoints and JMX exposure are disabled.
